...
questions = [
    ...
    {"id": 51, "topic": "Forgejo CI/CD", "question": "Что такое Forgejo?", "options": ['Платформа для хостинга Git-репозиториев (аналог GitHub)', 'CI/CD сервер', 'База данных', 'Веб-сервер'], "correct": "Платформа для хостинга Git-репозиториев (аналог GitHub)"},
    {"id": 52, "topic": "Forgejo CI/CD", "question": "Какой сервис используется в качестве CI/CD в связке с Forgejo?", "options": ['Jenkins', 'GitLab CI', 'Drone CI', 'GitHub Actions'], "correct": "Drone CI"},
    {"id": 53, "topic": "Forgejo CI/CD", "question": "Какой протокол использует Forgejo для уведомления Drone о новых коммитах?", "options": ['SSH', 'Webhook', 'FTP', 'SMTP'], "correct": "Webhook"},
    {"id": 54, "topic": "Forgejo CI/CD", "question": "На каком порту работает Drone Server в настроенной конфигурации?", "options": ['80', '443', '8000', '8080'], "correct": "8000"},
    {"id": 55, "topic": "Forgejo CI/CD", "question": "Какая переменная окружения используется для связи Drone Server и Drone Runner?", "options": ['DRONE_RPC_SECRET', 'DRONE_TOKEN', 'DRONE_KEY', 'DRONE_PASSWORD'], "correct": "DRONE_RPC_SECRET"},
    {"id": 56, "topic": "Forgejo CI/CD", "question": "Что делает Drone Runner?", "options": ['Запускает пайплайны', 'Хранит код', 'Отправляет уведомления', 'Управляет пользователями'], "correct": "Запускает пайплайны"},
    {"id": 57, "topic": "Forgejo CI/CD", "question": "Какой файл конфигурации используется в Drone CI?", "options": ['.drone.yml', '.woodpecker.yml', '.github/workflows', '.gitlab-ci.yml'], "correct": ".drone.yml"},
    {"id": 58, "topic": "Forgejo CI/CD", "question": "Какой образ Docker используется для выполнения SSH-команд в пайплайне?", "options": ['alpine', 'ubuntu', 'appleboy/drone-ssh', 'curlimages/curl'], "correct": "appleboy/drone-ssh"},
    {"id": 59, "topic": "Forgejo CI/CD", "question": "Какой образ используется для отправки Telegram-уведомлений в пайплайне?", "options": ['appleboy/drone-telegram', 'curlimages/curl', 'alpine', 'ubuntu'], "correct": "curlimages/curl"},
    {"id": 60, "topic": "Forgejo CI/CD", "question": "Какой секрет необходимо добавить в Drone для доступа к серверу по SSH?", "options": ['ssh_key', 'deploy_key', 'server_key', 'private_key'], "correct": "ssh_key"},
    {"id": 61, "topic": "Forgejo CI/CD", "question": "Какой секрет необходимо добавить в Drone для отправки сообщений в Telegram?", "options": ['telegram_token', 'bot_token', 'tg_token', 'api_key'], "correct": "telegram_token"},
    {"id": 62, "topic": "Forgejo CI/CD", "question": "Какой командой запускается Drone Server в Docker?", "options": ['docker run -d --name drone-server -p 8000:80 ...', 'docker start drone-server', 'systemctl start drone', 'drone server start'], "correct": "docker run -d --name drone-server -p 8000:80 ..."},
    {"id": 63, "topic": "Forgejo CI/CD", "question": "Какой командой перезапускается systemd сервис post-generator в пайплайне?", "options": ['systemctl restart post-generator', 'service post-generator restart', 'restart post-generator', 'docker restart post-generator'], "correct": "systemctl restart post-generator"},
    {"id": 64, "topic": "Forgejo CI/CD", "question": "Как проверить, что вебхук Forgejo работает?", "options": ['curl -X POST http://localhost:8000/hook', 'ping codeberg.org', 'ssh forgejo', 'docker logs drone'], "correct": "curl -X POST http://localhost:8000/hook"},
    {"id": 65, "topic": "Forgejo CI/CD", "question": "Что делать, если вебхук Drone не отвечает (код 405)?", "options": ['Перезапустить Drone Server', 'Поменять порт', 'Обновить конфиг', 'Переустановить Forgejo'], "correct": "Перезапустить Drone Server"},
    {"id": 66, "topic": "Forgejo CI/CD", "question": "Как настроить автоматическое восстановление Drone при падении?", "options": ['Cron + webhook_monitor.sh', 'Systemd сервис', 'Docker restart policy', 'Вручную'], "correct": "Cron + webhook_monitor.sh"},
    {"id": 67, "topic": "Forgejo CI/CD", "question": "Какой скрипт проверяет доступность вебхука каждые 5 минут?", "options": ['webhook_monitor.sh', 'healthcheck.sh', 'drone_check.sh', 'monitor.sh'], "correct": "webhook_monitor.sh"},
    {"id": 68, "topic": "Forgejo CI/CD", "question": "Какой домен используется для AI Post Generator?", "options": ['aigenerator.myvnc.com', 'ai.cuby.by', 'generator.ai', 'post.ai'], "correct": "aigenerator.myvnc.com"},
    {"id": 69, "topic": "Forgejo CI/CD", "question": "Как называется Telegram-бот для уведомлений о деплое?", "options": ['@SeRveDog_bot', '@AIPostBot', '@DeployBot', '@DroneBot'], "correct": "@SeRveDog_bot"},
    {"id": 70, "topic": "Forgejo CI/CD", "question": "Какой порт слушает Streamlit-приложение post-generator?", "options": ['8501', '8502', '8080', '8000'], "correct": "8502"},
    {"id": 71, "topic": "Forgejo CI/CD", "question": "Какой параметр systemd ограничивает использование CPU?", "options": ['CPUQuota', 'MemoryMax', 'LimitCPU', 'CPUMax'], "correct": "CPUQuota"},
    {"id": 72, "topic": "Forgejo CI/CD", "question": "Какой параметр systemd ограничивает использование памяти?", "options": ['MemoryMax', 'CPUQuota', 'LimitMemory', 'MemMax'], "correct": "MemoryMax"},
    {"id": 73, "topic": "Forgejo CI/CD", "question": "Как часто cron запускает webhook_monitor.sh?", "options": ['Каждые 5 минут', 'Каждую минуту', 'Каждый час', 'Раз в день'], "correct": "Каждые 5 минут"},
    {"id": 74, "topic": "Forgejo CI/CD", "question": "Что происходит при успешном деплое в Telegram?", "options": ['Приходит уведомление с галочкой', 'Ничего', 'Приходит ошибка', 'Перезапускается сервер'], "correct": "Приходит уведомление с галочкой"},
    {"id": 75, "topic": "Forgejo CI/CD", "question": "Какая команда перезапускает Drone Server в скрипте мониторинга?", "options": ['docker restart drone-server', 'systemctl restart drone', 'reboot', 'killall drone'], "correct": "docker restart drone-server"},
    {"id": 76, "topic": "Forgejo CI/CD", "question": "Какой метод HTTP используется для вебхука Forgejo?", "options": ['POST', 'GET', 'PUT', 'DELETE'], "correct": "POST"},
    {"id": 77, "topic": "Forgejo CI/CD", "question": "Какой URL используется для вебхука в Forgejo?", "options": ['http://103.125.216.110:8000/hook', 'http://localhost:8000/webhook', 'https://codeberg.org/hook', 'http://drone:8000/hook'], "correct": "http://103.125.216.110:8000/hook"},
    {"id": 78, "topic": "Forgejo CI/CD", "question": "Какая ветка триггерит деплой?", "options": ['master', 'main', 'develop', 'feature/*'], "correct": "master"},
    {"id": 79, "topic": "Forgejo CI/CD", "question": "Какой шаг в .drone.yml отвечает за деплой?", "options": ['deploy', 'build', 'test', 'notify'], "correct": "deploy"},
    {"id": 80, "topic": "Forgejo CI/CD", "question": "Что такое Forgejo?", "options": ['Платформа для хостинга Git', 'CI/CD сервер', 'База данных', 'Веб-сервер'], "correct": "Платформа для хостинга Git"},
    {"id": 81, "topic": "Forgejo CI/CD", "question": "Какой CI/CD сервер используется с Forgejo?", "options": ['Jenkins', 'Drone CI', 'GitLab CI', 'GitHub Actions'], "correct": "Drone CI"},
    {"id": 82, "topic": "Forgejo CI/CD", "question": "Какой протокол использует Forgejo для уведомления Drone?", "options": ['SSH', 'Webhook', 'FTP', 'SMTP'], "correct": "Webhook"},
    {"id": 83, "topic": "Git", "question": "Какая команда отменяет последний коммит, сохраняя изменения в рабочей директории?", "options": ['git reset --soft HEAD~1', 'git reset --hard HEAD~1', 'git revert HEAD', 'git checkout HEAD~1'], "correct": "git reset --soft HEAD~1"},
    {"id": 84, "topic": "Git", "question": "Что делает команда 'git revert'?", "options": ['Удаляет коммит из истории', 'Создаёт новый коммит, отменяющий изменения', 'Перемещает указатель ветки', 'Очищает рабочую директорию'], "correct": "Создаёт новый коммит, отменяющий изменения"},
    {"id": 85, "topic": "Kubernetes", "question": "Что такое Kubernetes?", "options": ['Оркестратор контейнеров', 'База данных', 'Язык программирования', 'CI/CD система'], "correct": "Оркестратор контейнеров"},
    {"id": 86, "topic": "Kubernetes", "question": "Что такое Pod в Kubernetes?", "options": ['Группа контейнеров', 'Физический сервер', 'Сеть', 'База данных'], "correct": "Группа контейнеров"},
    {"id": 87, "topic": "Kubernetes", "question": "Что делает kubectl?", "options": ['CLI для управления K8s', 'Сервер баз данных', 'Система мониторинга', 'Веб-сервер'], "correct": "CLI для управления K8s"},
    {"id": 88, "topic": "Kubernetes", "question": "Что такое Deployment?", "options": ['Ресурс для управления подами', 'База данных', 'Тип сети', 'Система мониторинга'], "correct": "Ресурс для управления подами"},
    {"id": 89, "topic": "Kubernetes", "question": "Что такое Service в K8s?", "options": ['Балансирует трафик к подам', 'База данных', 'Сеть', 'Тип пода'], "correct": "Балансирует трафик к подам"},
    {"id": 90, "topic": "Kubernetes", "question": "Что такое Ingress?", "options": ['Управляет внешним доступом к сервисам', 'Тип пода', 'База данных', 'Система мониторинга'], "correct": "Управляет внешним доступом к сервисам"},
    {"id": 91, "topic": "Kubernetes", "question": "Что такое ConfigMap?", "options": ['Хранит конфигурацию', 'База данных', 'Тип сети', 'Система мониторинга'], "correct": "Хранит конфигурацию"},
    {"id": 92, "topic": "Kubernetes", "question": "Что такое 'kubelet'?", "options": ['Агент на узле, управляющий подам', 'CLI для K8s', 'База данных', 'Система мониторинга'], "correct": "Агент на узле, управляющий подам"},
    {"id": 93, "topic": "Kubernetes", "question": "Что такое 'etcd'?", "options": ['Хранилище данных кластера K8s', 'База данных', 'Система логирования', 'Тип пода'], "correct": "Хранилище данных кластера K8s"},
    {"id": 94, "topic": "Kubernetes", "question": "Что такое readinessProbe в Kubernetes?", "options": ['Проверяет, готов ли под принимать трафик', 'Проверяет, жив ли контейнер', 'Проверяет диск', 'Проверяет сеть'], "correct": "Проверяет, готов ли под принимать трафик"},
    {"id": 95, "topic": "Kubernetes", "question": "Что такое livenessProbe в Kubernetes?", "options": ['Проверяет, жив ли контейнер, и перезапускает его при падении', 'Проверяет готовность к трафику', 'Проверяет диск', 'Проверяет сеть'], "correct": "Проверяет, жив ли контейнер, и перезапускает его при падении"},
    {"id": 96, "topic": "Linux", "question": "Как посмотреть логи systemd-сервиса в реальном времени?", "options": ['systemctl logs -f', 'journalctl -u <service> -f', 'tail -f /var/log/syslog', 'dmesg -w'], "correct": "journalctl -u <service> -f"},
    {"id": 97, "topic": "Linux", "question": "Как ограничить CPU для systemd-сервиса?", "options": ['CPULimit=80', 'CPUQuota=80%', 'LimitCPU=80', 'CPU=80%'], "correct": "CPUQuota=80%"},
    {"id": 98, "topic": "Linux", "question": "Как перезагрузить конфигурацию systemd после изменения файла?", "options": ['systemctl restart', 'systemctl reload', 'systemctl daemon-reload', 'systemctl update'], "correct": "systemctl daemon-reload"},
    {"id": 99, "topic": "Linux", "question": "Как посмотреть список всех активных сервисов systemd?", "options": ['systemctl list', 'systemctl status', 'systemctl list-units --type=service', 'systemctl show'], "correct": "systemctl list-units --type=service"},
    {"id": 100, "topic": "Linux", "question": "Какая команда показывает использование диска в человеко-читаемом формате?", "options": ['du -h', 'df -h', 'ls -lh', 'fdisk -l'], "correct": "df -h"},
    {"id": 101, "topic": "Linux", "question": "Как посмотреть, какие порты слушает сервер?", "options": ['netstat -tlnp', 'iptables -L', 'ss -s', 'tcpdump'], "correct": "netstat -tlnp"},
    {"id": 102, "topic": "Linux", "question": "Как найти процесс, который ест 100% CPU?", "options": ['htop (нажать P)', 'ls -l', 'ps aux', 'kill -9'], "correct": "htop (нажать P)"},
    {"id": 103, "topic": "Linux", "question": "Что делает команда 'chmod +x script.sh'?", "options": ['Удаляет файл', 'Делает файл исполняемым', 'Копирует файл', 'Переименовывает файл'], "correct": "Делает файл исполняемым"},
    {"id": 104, "topic": "Linux", "question": "Как посмотреть размер папки?", "options": ['ls -la', 'du -sh', 'df -h', 'find . -size'], "correct": "du -sh"},
    {"id": 105, "topic": "Linux", "question": "Как посмотреть историю команд в Bash?", "options": ['history', 'cat ~/.bash_history', 'Оба варианта', 'Ни один'], "correct": "Оба варианта"},
    {"id": 106, "topic": "Linux", "question": "Что такое inode?", "options": ['Структура с метаданными файла', 'IP адрес', 'Сетевое соединение', 'Тип файловой системы'], "correct": "Структура с метаданными файла"},
    {"id": 107, "topic": "Linux", "question": "Как применить права 755 к папке рекурсивно?", "options": ['chmod 755 *', 'chmod -R 755 ./', 'chmod 755 -R', 'chmod 755'], "correct": "chmod -R 755 ./"},
    {"id": 108, "topic": "Linux", "question": "Что такое cron?", "options": ['Сервис для мониторинга', 'Планировщик задач', 'Система логирования', 'База данных'], "correct": "Планировщик задач"},
    {"id": 109, "topic": "Linux", "question": "Как посмотреть последние 10 строк лога?", "options": ['head -10 file', 'tail -10 file', 'less file', 'grep -10 file'], "correct": "tail -10 file"},
    {"id": 110, "topic": "Linux", "question": "Что такое swap?", "options": ['Место на диске для расширения RAM', 'Тип процессора', 'Сетевое соединение', 'Системный вызов'], "correct": "Место на диске для расширения RAM"},
    {"id": 111, "topic": "Linux", "question": "Как убить процесс по имени?", "options": ['kill <pid>', 'pkill <name>', 'killall <name>', 'Оба варианта pkill и killall'], "correct": "Оба варианта pkill и killall"},
    {"id": 112, "topic": "Linux", "question": "Что делает команда 'grep -r pattern /path'?", "options": ['Ищет файлы по имени', 'Ищет паттерн в файлах рекурсивно', 'Удаляет файлы с паттерном', 'Заменяет паттерн'], "correct": "Ищет паттерн в файлах рекурсивно"},
    {"id": 113, "topic": "Linux", "question": "Как посмотреть загрузку CPU в реальном времени?", "options": ['top', 'htop', 'ps aux', 'Все варианты'], "correct": "Все варианты"},
    {"id": 114, "topic": "Linux", "question": "Что такое SSH?", "options": ['Протокол удалённого доступа', 'Сетевой файрвол', 'Система логирования', 'База данных'], "correct": "Протокол удалённого доступа"},
    {"id": 115, "topic": "Linux", "question": "Как создать systemd-сервис?", "options": ['Создать файл в /etc/systemd/system/*.service', 'Запустить systemd-create', 'Использовать init.d', 'Все варианты'], "correct": "Создать файл в /etc/systemd/system/*.service"},
    {"id": 116, "topic": "Linux", "question": "Что делает команда 'ps aux'?", "options": ['Показывает все процессы', 'Удаляет процессы', 'Создает процессы', 'Останавливает процессы'], "correct": "Показывает все процессы"},
    {"id": 117, "topic": "Linux", "question": "Как посмотреть версию ядра Linux?", "options": ['uname -r', 'kernel --version', 'version', 'cat /proc/version'], "correct": "uname -r"},
    {"id": 118, "topic": "Linux", "question": "Как посмотреть загрузку памяти?", "options": ['free -h', 'top', 'htop', 'Все варианты'], "correct": "Все варианты"},
    {"id": 119, "topic": "Linux", "question": "Что делает команда 'systemctl daemon-reload'?", "options": ['Перезагружает systemd', 'Перечитывает конфигурацию systemd', 'Перезапускает все сервисы', 'Останавливает systemd'], "correct": "Перечитывает конфигурацию systemd"},
    {"id": 120, "topic": "Linux", "question": "Какой лимит CPU установлен для Container Packer в systemd?", "options": ['50%', '80%', '100%', '200%'], "correct": "80%"},
    {"id": 121, "topic": "Linux", "question": "Какой лимит памяти установлен для Container Packer в systemd?", "options": ['256MB', '512MB', '800MB', '1GB'], "correct": "800MB"},
    {"id": 122, "topic": "Monitoring", "question": "Что такое Prometheus?", "options": ['Система мониторинга и метрик', 'База данных', 'Веб-сервер', 'CI/CD система'], "correct": "Система мониторинга и метрик"},
    {"id": 123, "topic": "Monitoring", "question": "Что такое Grafana?", "options": ['Инструмент визуализации дашбордов', 'Система логирования', 'База данных', 'CI/CD система'], "correct": "Инструмент визуализации дашбордов"},
    {"id": 124, "topic": "Monitoring", "question": "Что такое экспортёр в Prometheus?", "options": ['Сборщик метрик со сторонних систем', 'Дашборд', 'База данных', 'CI/CD система'], "correct": "Сборщик метрик со сторонних систем"},
    {"id": 125, "topic": "Monitoring", "question": "Что такое SLO?", "options": ['Service Level Objective — целевой уровень обслуживания', 'Система логирования', 'База данных', 'Тип метрики'], "correct": "Service Level Objective — целевой уровень обслуживания"},
    {"id": 126, "topic": "Monitoring", "question": "Что такое Alertmanager?", "options": ['Управляет уведомлениями в Prometheus', 'Система мониторинга', 'База данных', 'Веб-сервер'], "correct": "Управляет уведомлениями в Prometheus"},
    {"id": 127, "topic": "Monitoring", "question": "Что такое healthcheck?", "options": ['Проверка доступности сервиса', 'Система мониторинга', 'База данных', 'CI/CD система'], "correct": "Проверка доступности сервиса"},
    {"id": 128, "topic": "Monitoring", "question": "Что такое трейсинг (tracing)?", "options": ['Отслеживание запросов в распределённых системах', 'Система мониторинга', 'База данных', 'CI/CD система'], "correct": "Отслеживание запросов в распределённых системах"},
    {"id": 129, "topic": "Monitoring", "question": "Что такое 'metrics' в мониторинге?", "options": ['Числовые показатели состояния системы', 'Логи', 'Трейсы', 'События'], "correct": "Числовые показатели состояния системы"},
    {"id": 130, "topic": "Monitoring", "question": "Что такое 'dashboard'?", "options": ['Панель визуализации метрик', 'База данных', 'Система алертов', 'Сетевой протокол'], "correct": "Панель визуализации метрик"},
    {"id": 131, "topic": "Monitoring", "question": "Как часто cron проверяет вебхук в Container Packer?", "options": ['Каждую минуту', 'Каждые 5 минут', 'Каждые 15 минут', 'Каждый час'], "correct": "Каждые 5 минут"},
    {"id": 132, "topic": "Monitoring", "question": "Сколько времени занимает восстановление вебхука в Container Packer?", "options": ['1-2 секунды', '5-6 секунд', '10-11 секунд', 'Минута'], "correct": "5-6 секунд"},
    {"id": 133, "topic": "Monitoring", "question": "Сколько времени занимает восстановление приложения (Streamlit) в Container Packer?", "options": ['1-2 секунды', '5-6 секунд', '10-11 секунд', 'Минута'], "correct": "10-11 секунд"},
    {"id": 134, "topic": "Monitoring", "question": "Какой endpoint используется для healthcheck вебхука?", "options": ['/health', '/status', '/ping', '/check'], "correct": "/health"},
    {"id": 135, "topic": "My Cases", "question": "Как вы починили 502 Bad Gateway на Container Packer?", "options": ['Перезапустил Nginx', 'Ограничил CPU через CPUQuota=80% в systemd', 'Добавил больше памяти', 'Обновил Python'], "correct": "Ограничил CPU через CPUQuota=80% в systemd"},
    {"id": 136, "topic": "My Cases", "question": "Почему перестал работать Webhook и как вы исправили?", "options": ['GitHub изменил API', 'Порт 9000 заблокировал хостинг, перевел на HTTPS через Nginx', 'Слетели права доступа', 'Закончилось место'], "correct": "Порт 9000 заблокировал хостинг, перевел на HTTPS через Nginx"},
    {"id": 137, "topic": "My Cases", "question": "Какой уникальный опыт вы получили на линии 102?", "options": ['Работа с критически важной системой 24/7', 'Настройка маршрутизаторов', 'Программирование на C++', 'Администрирование Windows'], "correct": "Работа с критически важной системой 24/7"},
    {"id": 138, "topic": "My Cases", "question": "Какой стек использует Container Packer?", "options": ['Django + MySQL', 'Streamlit + Nginx + systemd', 'Node.js + MongoDB', 'PHP + Apache'], "correct": "Streamlit + Nginx + systemd"},
    {"id": 139, "topic": "My Cases", "question": "Как вы настроили мониторинг для Container Packer?", "options": ['Prometheus + Grafana', 'Beszel + Telegram-алерты', 'Zabbix', 'Nagios'], "correct": "Beszel + Telegram-алерты"},
    {"id": 140, "topic": "My Cases", "question": "Что вы внедрили в CRM Planfix?", "options": ['Учет ИТ-оборудования и ITSM-процессы', 'Чат-бот', 'Онлайн-магазин', 'Биллинг'], "correct": "Учет ИТ-оборудования и ITSM-процессы"},
    {"id": 141, "topic": "My Cases", "question": "Какой CI/CD вы используете в Container Packer?", "options": ['Jenkins', 'GitLab CI', 'GitHub Actions + Webhook', 'TeamCity'], "correct": "GitHub Actions + Webhook"},
    {"id": 142, "topic": "My Cases", "question": "Какой VPS провайдер заблокировал порт 9000?", "options": ['hoster.by', 'kamatera.com', 'fornex.com', 'digitalocean.com'], "correct": "fornex.com"},
    {"id": 143, "topic": "My Cases", "question": "Какой домен использует Container Packer?", "options": ['autolot25.ddns.net', 'packer.cuby.by', 'trainer.ddns.net', 'aigenerator.myvnc.com'], "correct": "packer.cuby.by"},
    {"id": 144, "topic": "My Cases", "question": "Как часто настроен ределивер в GitHub Actions?", "options": ['Каждый час', 'Каждые 6 часов', 'Раз в день', 'Раз в неделю'], "correct": "Каждые 6 часов"},
    {"id": 145, "topic": "My Cases", "question": "Какая итоговая надёжность системы Container Packer?", "options": ['90-95%', '95-97%', '97-99%', '99.9%'], "correct": "97-99%"},
    {"id": 146, "topic": "My Cases", "question": "Что вы использовали для асинхронного ответа вебхука?", "options": ['celery', 'threading', 'asyncio', 'multiprocessing'], "correct": "threading"},
    {"id": 147, "topic": "My Cases", "question": "Какой язык использует Auto Inventory System?", "options": ['Python (Flask)', 'Python (Django)', 'Node.js', 'Go'], "correct": "Python (Flask)"},
    {"id": 148, "topic": "My Cases", "question": "Какой домен у AI Post Generator?", "options": ['aigenerator.ddns.net', 'aigenerator.myvnc.com', 'ai.cuby.by', 'generator.ai'], "correct": "aigenerator.myvnc.com"},
    {"id": 149, "topic": "My Cases", "question": "Что вы используете для мониторинга серверов?", "options": ['Prometheus', 'Zabbix', 'Beszel', 'Datadog'], "correct": "Beszel"},
    {"id": 150, "topic": "My Cases", "question": "Какой домен у DevOps Interview Trainer?", "options": ['trainer.ddns.net', 'trainer.myvnc.com', 'devops.cuby.by', 'interview.ai'], "correct": "trainer.ddns.net"},
    {"id": 151, "topic": "My Cases", "question": "Сколько вопросов в DevOps Interview Trainer?", "options": ['50', '100', '150', '278'], "correct": "278"},
    {"id": 152, "topic": "My Cases", "question": "Какой стек использует DevOps Interview Trainer?", "options": ['Flask + Nginx', 'Django + Apache', 'Node.js + Express', 'Go + Nginx'], "correct": "Flask + Nginx"},
    {"id": 153, "topic": "My Cases", "question": "Что вы использовали для управления версиями Container Packer?", "options": ['Git', 'SVN', 'Mercurial', 'Perforce'], "correct": "Git"},
    {"id": 154, "topic": "My Cases", "question": "Какой сервер использует Container Packer?", "options": ['VPS на Fornex', 'AWS', 'Google Cloud', 'Azure'], "correct": "VPS на Fornex"},
    {"id": 155, "topic": "My Cases", "question": "Какой сервис используете для Telegram уведомлений?", "options": ['Telegram Bot API', 'Webhook', 'Long Polling', 'Все варианты'], "correct": "Telegram Bot API"},
    {"id": 156, "topic": "My Cases", "question": "Что используете для асинхронного вебхука?", "options": ['threading', 'asyncio', 'celery', 'multiprocessing'], "correct": "threading"},
    {"id": 157, "topic": "My Cases", "question": "Какая команда перезапускает сервис container-packer?", "options": ['sudo systemctl restart container-packer', 'sudo service restart', 'sudo restart', 'sudo reload'], "correct": "sudo systemctl restart container-packer"},
    {"id": 158, "topic": "My Cases", "question": "Что делает скрипт deploy.sh в Container Packer?", "options": ['git pull + systemctl restart', 'docker build', 'npm install', 'python setup.py'], "correct": "git pull + systemctl restart"},
    {"id": 159, "topic": "My Cases", "question": "Как часто запускается ределивер в GitHub Actions для Container Packer?", "options": ['Каждый час', 'Каждые 6 часов', 'Раз в день', 'Раз в неделю'], "correct": "Каждые 6 часов"},
    {"id": 160, "topic": "My Cases", "question": "На каком языке написан скрипт ределивера в Container Packer?", "options": ['Python', 'Bash', 'JavaScript (Node.js, Octokit)', 'Go'], "correct": "JavaScript (Node.js, Octokit)"},
    {"id": 161, "topic": "My Cases", "question": "Какой статус код считается неудачным для вебхука в ределивере?", "options": ['200-299', '300-399', 'Меньше 200 или больше 299', 'Только 500'], "correct": "Меньше 200 или больше 299"},
    {"id": 162, "topic": "My Cases", "question": "Какой флаг в ределивере проверяет, что доставка ещё не переотправлялась?", "options": ['redelivered', 'retry', 'redelivery', 'attempt'], "correct": "redelivery"},
    {"id": 163, "topic": "My Cases", "question": "При каком заполнении диска приходит Telegram-уведомление в Container Packer?", "options": ['Более 50%', 'Более 75%', 'Более 85%', 'Более 95%'], "correct": "Более 85%"},
    {"id": 164, "topic": "My Cases", "question": "Как называется Telegram-бот для уведомлений в Container Packer?", "options": ['ContainerBot', 'PackerBot', 'AutoInventoryBot', 'DeployBot'], "correct": "AutoInventoryBot"},
    {"id": 165, "topic": "My Cases", "question": "Какой порт слушает вебхук в Container Packer?", "options": ['80', '443', '8080', '9000'], "correct": "9000"},
    {"id": 166, "topic": "My Cases", "question": "Какой порт слушает Streamlit-приложение в Container Packer?", "options": ['80', '443', '8501', '9000'], "correct": "8501"},
    {"id": 167, "topic": "My Cases", "question": "Какой домен у DevOps Interview Trainer?", "options": ['trainer.ddns.net', 'trainer.myvnc.com', 'devops.cuby.by', 'interview.ai'], "correct": "trainer.ddns.net"},
    {"id": 168, "topic": "My Cases", "question": "Какой фреймворк используется в DevOps Interview Trainer?", "options": ['Django', 'Flask', 'FastAPI', 'Express'], "correct": "Flask"},
    {"id": 169, "topic": "My Cases", "question": "Какой скрипт отвечает за деплой в Container Packer?", "options": ['deploy.sh', 'webhook.py', 'start.sh', 'update.sh'], "correct": "deploy.sh"},
    {"id": 170, "topic": "My Cases", "question": "Какая команда перезапускает вебхук при сбое?", "options": ['sudo systemctl restart webhook', 'sudo restart webhook', 'sudo webhook restart', 'sudo service webhook restart'], "correct": "sudo systemctl restart webhook"},
    {"id": 171, "topic": "My Cases", "question": "Какой сервис обрабатывает асинхронные запросы от GitHub?", "options": ['Flask', 'Streamlit', 'Webhook на Python', 'Nginx'], "correct": "Webhook на Python"},
    {"id": 172, "topic": "My Cases", "question": "Что делает параметр MemoryMax в systemd?", "options": ['Ограничивает память сервиса', 'Увеличивает память', 'Задаёт swap', 'Ничего не делает'], "correct": "Ограничивает память сервиса"},
    {"id": 173, "topic": "My Cases", "question": "Какой алгоритм используется для упаковки коробок?", "options": ['3D bin packing', '2D bin packing', 'Genetic algorithm', 'Greedy algorithm'], "correct": "3D bin packing"},
    {"id": 174, "topic": "My Cases", "question": "Какой порт использует Nginx для HTTPS?", "options": ['80', '443', '8080', '8443'], "correct": "443"},
    {"id": 175, "topic": "Networks", "question": "Чем отличается TCP от UDP?", "options": ['TCP быстрее, UDP надёжнее', 'TCP надежный с подтверждением, UDP быстрый без подтверждения', 'TCP для веб-сайтов, UDP для файлов', 'Одинаковые'], "correct": "TCP надежный с подтверждением, UDP быстрый без подтверждения"},
    {"id": 176, "topic": "Networks", "question": "Что такое reverse proxy?", "options": ['Сервер для кеширования', 'Сервер, перенаправляющий запросы на внутренние сервера', 'Балансировщик нагрузки', 'Файрвол'], "correct": "Сервер, перенаправляющий запросы на внутренние сервера"},
    {"id": 177, "topic": "Networks", "question": "Какой порт у HTTPS?", "options": ['80', '443', '22', '8080'], "correct": "443"},
    {"id": 178, "topic": "Networks", "question": "Что такое DNS?", "options": ['Превращает домен в IP-адрес', 'Система шифрования', 'База данных', 'Язык программирования'], "correct": "Превращает домен в IP-адрес"},
    {"id": 179, "topic": "Networks", "question": "Что происходит при вводе https://site.com?", "options": ['DNS → TCP → TLS → HTTP', 'HTTP → DNS → TCP', 'TLS → TCP → DNS', 'TCP → HTTP → DNS'], "correct": "DNS → TCP → TLS → HTTP"},
    {"id": 180, "topic": "Networks", "question": "Чем отличается HTTP от HTTPS?", "options": ['HTTPS использует порт 80', 'HTTPS шифрует данные (TLS/SSL)', 'HTTP быстрее', 'Это одно и то же'], "correct": "HTTPS шифрует данные (TLS/SSL)"},
    {"id": 181, "topic": "Networks", "question": "Что такое 502 Bad Gateway?", "options": ['Сервер не найден', 'Шлюз не получил ответ от бэкенда', 'Нет доступа', 'Слишком много запросов'], "correct": "Шлюз не получил ответ от бэкенда"},
    {"id": 182, "topic": "Networks", "question": "Какой порт у SSH?", "options": ['22', '23', '21', '25'], "correct": "22"},
    {"id": 183, "topic": "Networks", "question": "Что такое VLAN?", "options": ['Виртуальная локальная сеть', 'Тип маршрутизатора', 'Протокол шифрования', 'Система мониторинга'], "correct": "Виртуальная локальная сеть"},
    {"id": 184, "topic": "Networks", "question": "Что такое NAT?", "options": ['Преобразование сетевых адресов', 'Протокол туннелирования', 'Система обнаружения вторжений', 'Тип файрвола'], "correct": "Преобразование сетевых адресов"},
    {"id": 185, "topic": "Networks", "question": "Как проверить, открыт ли порт на сервере?", "options": ['telnet host port', 'nc -zv host port', 'nmap host -p port', 'Все варианты'], "correct": "Все варианты"},
    {"id": 186, "topic": "Networks", "question": "Что такое CIDR?", "options": ['Формат записи IP-сетей', 'Протокол маршрутизации', 'Тип DNS записи', 'Система шифрования'], "correct": "Формат записи IP-сетей"},
    {"id": 187, "topic": "Networks", "question": "Какой протокол используется для ping?", "options": ['TCP', 'UDP', 'ICMP', 'ARP'], "correct": "ICMP"},
    {"id": 188, "topic": "Networks", "question": "Что такое SSL/TLS?", "options": ['Протокол шифрования', 'Система аутентификации', 'Сетевой протокол', 'База данных'], "correct": "Протокол шифрования"},
    {"id": 189, "topic": "Networks", "question": "Что такое HTTP/2?", "options": ['Новая версия HTTP с мультиплексированием', 'Система кеширования', 'Протокол шифрования', 'Тип сервера'], "correct": "Новая версия HTTP с мультиплексированием"},
    {"id": 190, "topic": "Networks", "question": "Что такое 'subnet' (подсеть)?", "options": ['Логическое разделение IP-сети', 'Тип маршрутизатора', 'Сетевой протокол', 'Система шифрования'], "correct": "Логическое разделение IP-сети"},
    {"id": 191, "topic": "Networks", "question": "Что такое 'TLS handshake'?", "options": ['Обмен ключами для шифрования', 'Тип соединения', 'Сетевой протокол', 'Аутентификация'], "correct": "Обмен ключами для шифрования"},
    {"id": 192, "topic": "Scripting", "question": "Какая библиотека Python используется для создания веб-приложений в Auto Inventory System?", "options": ['Django', 'Flask', 'FastAPI', 'Tornado'], "correct": "Flask"},
    {"id": 193, "topic": "Scripting", "question": "Какой фреймворк Python используется в AI Post Generator для веб-интерфейса?", "options": ['Flask', 'Django', 'Streamlit', 'FastAPI'], "correct": "Streamlit"},
    {"id": 194, "topic": "Scripting", "question": "Как в Python создать асинхронный ответ вебхука, чтобы не блокировать GitHub?", "options": ['asyncio', 'threading', 'multiprocessing', 'celery'], "correct": "threading"},
    {"id": 195, "topic": "Scripting", "question": "Какая библиотека используется для работы с Telegram Bot API в Container Packer?", "options": ['requests', 'python-telegram-bot', 'telebot', 'Все варианты'], "correct": "Все варианты"},
    {"id": 196, "topic": "Scripting", "question": "Как в Python отправить HTTP-запрос к API?", "options": ['requests.get()', 'urllib.request', 'http.client', 'Все варианты'], "correct": "Все варианты"},
    {"id": 197, "topic": "Scripting", "question": "Какая библиотека используется в ределивере для работы с GitHub API?", "options": ['axios', 'requests', 'Octokit', 'fetch'], "correct": "Octokit"},
    {"id": 198, "topic": "Scripting", "question": "Как в JavaScript (Node.js) установить пакет через npm?", "options": ['npm install <package>', 'npm add <package>', 'npm get <package>', 'npm download <package>'], "correct": "npm install <package>"},
    {"id": 199, "topic": "Scripting", "question": "Какой инструмент используется для запуска JavaScript-скрипта в ределивере?", "options": ['node script.js', 'npm run script', 'js script.js', 'javascript script.js'], "correct": "node script.js"},
    {"id": 200, "topic": "Scripting", "question": "Какая библиотека Python используется для мониторинга и проверки здоровья вебхука?", "options": ['requests', 'urllib', 'http.client', 'Все варианты'], "correct": "Все варианты"},
    {"id": 201, "topic": "Scripting", "question": "Как в Python получить переменные окружения?", "options": ["os.environ.get('KEY')", "os.getenv('KEY')", 'Оба варианта', 'Ни один'], "correct": "Оба варианта"},
    {"id": 202, "topic": "Scripting", "question": "Как в Node.js получить переменные окружения?", "options": ['process.env.KEY', 'system.env.KEY', "os.getenv('KEY')", 'environment.KEY'], "correct": "process.env.KEY"},
    {"id": 203, "topic": "Scripting", "question": "Как в Python отправить сообщение в Telegram через Bot API?", "options": ['requests.post(url, json=data)', 'telegram.send()', 'bot.send_message()', 'api.call()'], "correct": "requests.post(url, json=data)"},
    {"id": 204, "topic": "Scripting", "question": "Какая структура данных в Python используется для хранения вопросов в тренажёре?", "options": ['Список словарей (list of dicts)', 'Массив', 'JSON', 'XML'], "correct": "Список словарей (list of dicts)"},
    {"id": 205, "topic": "Scripting", "question": "Как в Python запустить shell-команду и получить вывод?", "options": ['subprocess.run()', 'os.system()', 'os.popen()', 'Все варианты'], "correct": "Все варианты"},
    {"id": 206, "topic": "Scripting", "question": "Как в Python создать простой HTTP-сервер для вебхука?", "options": ['http.server', 'flask', 'fastapi', 'Все варианты'], "correct": "Все варианты"},
    {"id": 207, "topic": "Scripting", "question": "Как в Node.js выполнить HTTP-запрос к API?", "options": ['axios.get()', 'fetch()', 'http.request()', 'Все варианты'], "correct": "Все варианты"},
    {"id": 208, "topic": "Scripting", "question": "Какой менеджер пакетов используется в проекте ределивера?", "options": ['npm', 'yarn', 'pnpm', 'Все варианты'], "correct": "npm"},
    {"id": 209, "topic": "Scripting", "question": "Как в Python обработать ошибку при HTTP-запросе?", "options": ['try/except', 'if response.status_code != 200', 'Оба варианта', 'Ни один'], "correct": "Оба варианта"},
    {"id": 210, "topic": "Scripting", "question": "Как в JavaScript распарсить JSON-ответ от API?", "options": ['JSON.parse()', 'response.json()', 'Оба варианта', 'Ни один'], "correct": "Оба варианта"},
    {"id": 211, "topic": "Scripting", "question": "Какой фреймворк Python используется в DevOps Interview Trainer?", "options": ['Django', 'Flask', 'FastAPI', 'Streamlit'], "correct": "Flask"},
    {"id": 212, "topic": "Security", "question": "Что такое Fail2ban?", "options": ['Блокирует IP после неудачных попыток входа', 'Файрвол', 'Система мониторинга', 'Антивирус'], "correct": "Блокирует IP после неудачных попыток входа"},
    {"id": 213, "topic": "Security", "question": "Как запретить вход по паролю в SSH?", "options": ['PasswordAuthentication no', 'PermitRootLogin no', 'AllowUsers no', 'DisablePassword yes'], "correct": "PasswordAuthentication no"},
    {"id": 214, "topic": "Security", "question": "Что такое SSL/TLS?", "options": ['Протокол шифрования', 'Система мониторинга', 'База данных', 'CI/CD система'], "correct": "Протокол шифрования"},
    {"id": 215, "topic": "Security", "question": "Что такое Let's Encrypt?", "options": ['Бесплатный центр сертификации SSL', 'Антивирус', 'Файрвол', 'CI/CD система'], "correct": "Бесплатный центр сертификации SSL"},
    {"id": 216, "topic": "Security", "question": "Что такое DDoS атака?", "options": ['Перегрузка сервера запросами', 'Кража данных', 'Вирус', 'Атака на пароль'], "correct": "Перегрузка сервера запросами"},
    {"id": 217, "topic": "Security", "question": "Как защитить SSH?", "options": ['Использовать ключи, запретить пароль, сменить порт', 'Никак', 'Поставить антивирус', 'Выключить сеть'], "correct": "Использовать ключи, запретить пароль, сменить порт"},
    {"id": 218, "topic": "Security", "question": "Что такое OWASP?", "options": ['Стандарт безопасности веб-приложений', 'Антивирус', 'База данных', 'CI/CD система'], "correct": "Стандарт безопасности веб-приложений"},
    {"id": 219, "topic": "Security", "question": "Что такое 'HTTPS'?", "options": ['HTTP с шифрованием', 'Сетевой протокол', 'База данных', 'Система мониторинга'], "correct": "HTTP с шифрованием"},
    {"id": 220, "topic": "Security", "question": "Что такое 'API key'?", "options": ['Ключ для аутентификации API', 'Пароль', 'Сертификат', 'Токен'], "correct": "Ключ для аутентификации API"},
    {"id": 221, "topic": "Security", "question": "Какой порт закрыт от внешнего мира в Container Packer?", "options": ['443', '80', '8501', '9000'], "correct": "8501"},
    {"id": 222, "topic": "Systemd", "question": "Что делает параметр StartLimitIntervalSec в systemd?", "options": ['Задаёт интервал для подсчёта перезапусков', 'Задаёт таймаут запуска', 'Ограничивает CPU', 'Ограничивает память'], "correct": "Задаёт интервал для подсчёта перезапусков"},
    {"id": 223, "topic": "Systemd", "question": "Что делает параметр StartLimitBurst в systemd?", "options": ['Максимальное число перезапусков за интервал', 'Скорость запуска', 'Приоритет процесса', 'Лимит памяти'], "correct": "Максимальное число перезапусков за интервал"},
    {"id": 224, "topic": "Terraform", "question": "Что такое Terraform?", "options": ['Инструмент IaC (Infrastructure as Code)', 'CI/CD система', 'База данных', 'Веб-сервер'], "correct": "Инструмент IaC (Infrastructure as Code)"},
    {"id": 225, "topic": "Terraform", "question": "Что такое 'terraform plan'?", "options": ['Показывает изменения до применения', 'Применяет изменения', 'Удаляет инфраструктуру', 'Создает план работ'], "correct": "Показывает изменения до применения"},
    {"id": 226, "topic": "Terraform", "question": "Что такое состояние (state) в Terraform?", "options": ['Файл с информацией о ресурсах', 'Система мониторинга', 'База данных', 'Логи'], "correct": "Файл с информацией о ресурсах"},
    {"id": 227, "topic": "Terraform", "question": "Чем отличается Terraform от Ansible?", "options": ['Terraform для управления инфраструктурой, Ansible для конфигурации', 'Они одинаковые', 'Terraform для конфигурации, Ansible для инфраструктуры', 'Ни чем'], "correct": "Terraform для управления инфраструктурой, Ansible для конфигурации"},
    {"id": 228, "topic": "Terraform", "question": "Что такое провайдер в Terraform?", "options": ['Плагин для взаимодействия с API (AWS, GCP, Azure)', 'Тип ресурса', 'Система мониторинга', 'База данных'], "correct": "Плагин для взаимодействия с API (AWS, GCP, Azure)"},
    {"id": 229, "topic": "Terraform", "question": "Как импортировать существующий ресурс?", "options": ['terraform import <address> <id>', 'terraform add', 'terraform create', 'terraform load'], "correct": "terraform import <address> <id>"},
    {"id": 230, "topic": "Terraform", "question": "Что делает 'terraform destroy'?", "options": ['Уничтожает всю инфраструктуру', 'Сохраняет состояние', 'Применяет изменения', 'Показывает план'], "correct": "Уничтожает всю инфраструктуру"}
]
, "correct": "journalctl -u <service> -f"},
